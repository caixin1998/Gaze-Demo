...
model_urls = {
    ...
    'hrnet64': 'hrnetv2_w64_imagenet_pretrained.pth',
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self,
        ...
        # stem network
        # stem net
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=stride[0], padding=1,
                               bias=False)
        self.bn1 = self.norm_layer(64)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=stride[1], padding=1,
                               bias=False)
        ...
        self.atp_modules = self._make_atp(pre_stage_channels)
    ...
    def _make_atp(self, pre_stage_channels):
        # patch_size = [56, 28, 14, 7]
        ...
        for i, channels in enumerate(pre_stage_channels):
            atp_module = ATP(
                (patch_size[i] >> self.half, patch_size[i]),
                channels,
                2048 if self.merge == "add" else 512
            )
            atp_modules.append(atp_module)
        atp_modules = nn.ModuleList(atp_modules)
        return atp_modules
    ...
    def forward(self, x):
        ...
        for i in range(len(y_list)):
            output_dicts.append(self.atp_modules[i](y_list[i]))
  
        y = output_dicts[0]["feature"]
        if self.merge == "add":
            for i in range(len(output_dicts) - 1):
                y = output_dicts[i+1]["feature"] + y
        elif self.merge == "concat":
            for i in range(len(output_dicts) - 1):
                y = torch.cat((y, output_dicts[i+1]["feature"]), 1)
        elif self.merge == "transformer":
            features = []
            for i in range(len(output_dicts)):
                features.append(output_dicts[i]["feature"].view(output_dicts[i]["feature"].shape[0], 1, -1))
            x_seq = torch.cat(features, 1)
            y = self.trans_layer(x_seq).view(x_seq.shape[0], -1)

        out_dict = {"feature":y}
        for i, output_dict in enumerate(output_dicts):
            out_dict["attn_%d"%i] = output_dict["attn"]
        return out_dict
...
def hratp18(pretrained=True, progress=True, **kwargs):
    r"""HRNet-18 model
    """
...
def hratp32(pretrained=True, progress=True, **kwargs):
    r"""HRNet-32 model
    """
...
def hratp48(pretrained=True, progress=True, **kwargs):
    r"""HRNet-48 model
    """
