...
cams = os.listdir(dataset_path)
for cam_id in cams: 
    persons = os.listdir(os.path.join(dataset_path, cam_id))
    persons.sort()
    ...
    for person in persons:
        os.makedirs(os.path.join(visualize_path, cam_id, person), exist_ok = True)

        out_dict = {}
        with h5py.File(os.path.join(data_path, cam_id, person + ".h5"),"r") as f:
            ...
            for i in range(num_entries):
                ...
                if i % 500 == 0:
                    ...
                    cv.imwrite(os.path.join(visualize_path, cam_id, person, '%05d.png'%i), cv.cvtColor(vis_img, cv.COLOR_RGB2BGR))
            with h5py.File(os.path.join(out_path, cam_id, person + ".h5"),"w") as g:
